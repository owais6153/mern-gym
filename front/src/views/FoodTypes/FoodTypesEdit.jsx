...
import { FoodTypesViewAction, FoodTypesUpdateAction } from "../../redux/actions/foodTypesAction";
import { connect } from "react-redux"
import Loder from "../../components/Loder/Loder";

function FoodTypesEdit (props) {
    const { id } = useParams();

    const location = useLocation();
    const navigate = useNavigate()
    const [error, setError] = useState({});
    const [FoodTypes, FoodTypesData] = useState({
    ...
    useEffect(()=>{
      props.FoodTypesViewAction(id).then(response => {});
    } , [])
    useEffect(()=>{
      if(props.food_type) {
        FoodTypesData((currentState) => ({
          ...currentState,
          ...props.food_type,
        }));
      }
    ...
    const FreatefoodTypesFormSubmitHandle = (e) => {
      e.preventDefault();
      let formData = new FormData(e.target);
      props.FoodTypesUpdateAction(formData, id, navigate).then(response => {});
    }
    return (
        <>
            <div className="content">
                              ...
                              <Col xl="4">
                                  <div className="d-flex">
                                      ...
                                      <div className="pl-3 flex-shrink-1 w-100 d-flex align-items-center">
                                          <div className="title">
                                              <h5 className="card-title font-weight-bold">Edit Food Type</h5>
                ...
                <Card className="guide-card">
                    <CardBody>
                        <form onSubmit={FreatefoodTypesFormSubmitHandle}>
                            <div className="form-row">
                                <FormGroup className="col-lg-6">
                                    <Label for="title">Title</Label>
                                    <Input type="text" name="title" id="title" placeholder="Enter title"
                                          value={FoodTypes.title} onChange={handleChange} 
                                          required invalid={error.title ? true:false}
                                        />
...
const mapStateToProps = state => ({
  food_type: state.foodTypesReducer.food_type,
  errors: state.commonReducer.errors,
  show_loder: state.commonReducer.show_loder,
})

const mapDispatchToProps = (dispatch) => ({
  FoodTypesViewAction: (id) => dispatch( FoodTypesViewAction(id) ),
  FoodTypesUpdateAction: (data, id, navigate) => dispatch( FoodTypesUpdateAction(data, id, navigate) ),
});

export default connect(mapStateToProps, mapDispatchToProps)(FoodTypesEdit)