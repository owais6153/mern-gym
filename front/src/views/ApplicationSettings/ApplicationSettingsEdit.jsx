...
import { ApplicationSettingsViewAction, ApplicationSettingsUpdateAction } from "../../redux/actions/applicationSettingsAction";
import { connect } from "react-redux"

function ApplicationSettingsEdit (props) {
    const { id } = useParams();
    const location = useLocation();
    const navigate = useNavigate()
    const [error, setError] = useState({});
    const [ApplicationSettings, ApplicationSettingsData] = useState({
    ...
    useEffect(()=>{
      props.ApplicationSettingsViewAction(id).then(response => {});
    } , [])
    useEffect(()=>{
      if(props.application_setting) {
        ApplicationSettingsData((currentState) => ({
          ...currentState,
          ...props.application_setting,
        }));
      }
    } , [props.application_setting])
    ...
    const CreateApplicationSettingsFormSubmitHandle = (e) => {
      ...
      props.ApplicationSettingsUpdateAction(formData, ApplicationSettings.id, navigate).then(response => {});
    }
        ...
        <>
            <div className="content">
                              ...
                              <Col xl="4">
                                  <div className="d-flex">
                                      ...
                                      <div className="pl-3 flex-shrink-1 w-100 d-flex align-items-center">
                                          <div className="title">
                                              <h5 className="card-title font-weight-bold">Edit Settings</h5>
                                              <ul className="breadCrumbList">
                                                  <li><a path="/admin/dashboard" >Home</a></li>
                                                  <li><a path="/admin/Users" >App Content</a></li>
                                                  <li><a path="/admin/Users" >Edit Settings</a></li>
                                              </ul>
                                          </div>
                                      </div>
                                  </div>
                ...
                <Card className="guide-card">
                    <CardBody>
                        <form onSubmit={CreateApplicationSettingsFormSubmitHandle}>
                            <div className="form-row">
                                <FormGroup className="col-lg-6">
                                    <Label for="title">Title</Label>
                                    <Input type="text" name="title" id="title" placeholder="Enter title"
                                          value={ApplicationSettings.title} onChange={handleChange} 
                                ...
                                <FormGroup className="col-lg-6">
                                    ...
                                    <Input type="text" name="slug" id="slug" placeholder="Enter display order"
                                          value={ApplicationSettings.slug} onChange={handleChange} 
                                          invalid={error.slug ? true:false}
                                        />
                                ...
                                <FormGroup className="col-lg-6">
                                    ...
                                    <Input type="text" name="value" id="value" placeholder="Enter value"
                                          value={ApplicationSettings.value} onChange={handleChange} 
                                          invalid={error.value ? true:false}
                                        />
                                ...
                                <div className="col-lg-6">
                                    ...
                                    <FormGroup check inline className="form-check-radio">
                                        <Label className="form-check-label">
                                            <Input type="radio" name="status" onChange={handleChange} value={1} invalid={error.status ? true:false} checked={ApplicationSettings.status == "1" && true} />
                                            ON
                                    ...
                                    <FormGroup check inline className="form-check-radio">
                                        <Label className="form-check-label">
                                            <Input type="radio" name="status" onChange={handleChange} value={0} invalid={error.status ? true:false} checked={ApplicationSettings.status == "0" || ApplicationSettings.status == null && true} />
                                            OFF
                                            <span className="form-check-sign"></span>
                                        </Label>
                                    </FormGroup>
...
const mapStateToProps = state => ({
  ...
  application_setting: state.applicationSettingsReducer.application_setting,
})

const mapDispatchToProps = (dispatch) => ({
  ApplicationSettingsUpdateAction: (data, id, navigate) => dispatch( ApplicationSettingsUpdateAction(data, id, navigate) ),
  ApplicationSettingsViewAction: (id) => dispatch( ApplicationSettingsViewAction(id) ),
});

export default connect(mapStateToProps, mapDispatchToProps)(ApplicationSettingsEdit)