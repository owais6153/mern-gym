...
function VideoUploder (props) {
  const [imageUrl, setImageUrl] = useState(null);

  useEffect(()=>{
    setImageUrl(props.video_url)
  } , [props.video_url])

  return (
    <FormGroup className="col-lg-6">
        <Label for="title">Video Thumbnail</Label>
        <div className="tpfupload">
            <div className="fileBg">
              
                {imageUrl ? 
                    <div>
                      <video controls autoplay alt="not fount" width={"250px"} 
                ...
                <Input type="file" name={props.field_name?props.field_name:"video"} id={props.field_name?props.field_name:'video'} 
                  onChange={(event) => {
                    setImageUrl(event.target.files[0]);
                  }}
                  invalid={props.error.image_url ? true:false}
                />
            </div>
            {
            !imageUrl && 
